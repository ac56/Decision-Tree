...
def find_split(train_df):
	...
	for i in train_df.columns:
		...
		if(current_best_split == len(train_df) - 1 ):
			continue
		
		if ((curr_split_entropy < least_curr_entropy) and (first == False)):
			least_curr_entropy = curr_split_entropy
			best_split['attr'] = i
			best_split['value'] = current_df[i].iloc[current_best_split]
...
def decision_tree_learning(train_df, depth):
	global decided_splits_list
	global label
	print('The train dataframe is:')
	print(train_df)
	print('\n')
	if(train_df.loc[:,label].var()==0):
		temp_list = train_df[label]
		print('temp_list:')
		print(temp_list)
		return ({'attr' : label , 'value' : temp_list[0], 'left' : None, 'right' : None, 'leaf' : True }, depth)
	else:

		print('ready to enter find split function...\n')
...
start = time.time()
print ("hello")
clean_array = np.loadtxt('small_dataset.txt')
